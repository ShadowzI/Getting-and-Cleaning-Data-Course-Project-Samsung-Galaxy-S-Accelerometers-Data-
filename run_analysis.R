...
names(Data)<-gsub("BodyBody", "Body", names(extractedData))
names(Data)<-gsub("Acc", "Accelerometer", names(extractedData))
names(Data)<-gsub("gravity", "Gravity", names(extractedData))
names(Data)<-gsub("Gyro", "Gyroscope", names(extractedData))
names(Data)<-gsub("tBody", "TimeBody", names(extractedData))
names(Data)<-gsub("Mag", "Magnitude", names(extractedData))
names(Data)<-gsub("^t", "Time", names(extractedData))
names(Data)<-gsub("^f", "Frequency", names(extractedData))
names(Data)<-gsub("-mean()", "Mean", names(extractedData), ignore.case = TRUE)
names(Data)<-gsub("-std()", "STD", names(extractedData), ignore.case = TRUE)
names(Data)<-gsub("-freq()", "Frequency", names(extractedData), ignore.case = TRUE)
names(Data)<-gsub("angle", "Angle", names(extractedData))
